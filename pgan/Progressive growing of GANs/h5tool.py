...
class HDF5Exporter:
    def __init__(self, h5_filename, resolution, channels=20):
...
def create_coupe256_channel_last(h5_filename):
    import pickle
    with open('./pickles/dataset_2d_256.pickle', 'rb') as handle:
        data = pickle.load(handle)
    print('Creating coupe256 channel last dataset %s' % (h5_filename))
    num_images = 401

    if len(data) != num_images:
        print('Error: Expected to find %d images in Coupe Dataset' % (num_images))
        return
    
    h5 = HDF5Exporter(h5_filename, 256, 20)
    for idx in range(num_images):
        print('%d / %d\r' % (idx, num_images), end=' ')
        img = data[idx]
        assert img.shape == (256, 256, 20)
        h5.add_images_channel_last(img[np.newaxis])
...
if __name__ == "__main__":
    ...
    create_coupe256_channel_last('datasets/coupe256.h5')
